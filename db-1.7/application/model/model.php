			...
			if (is_array($keyword) && count($keyword) > 0 && is_array($searchfield) && count($searchfield) > 0) {
				foreach ($keyword as $value) {
					...
					foreach ($searchfield as $key) {
						$row[] = sprintf("(%s COLLATE utf8_unicode_ci LIKE '%%%s%%')", $key, $this->quote($value));
					}
...
//もとショップ用　historyからCSV吐き出し用データ取得
	function findAll_csv($keyword = null, $sort = null, $desc = 0, $searchfield = null) {
		...
		if (is_array($array) && count($array) > 0) {
			$where = "WHERE ".$where."AND ".implode(" AND ", $array);
		}
		$query = sprintf("SELECT * FROM customer_customer %s %s", $where, $order);
//	var_dump($query);
		return $this->fetchAll($query);
