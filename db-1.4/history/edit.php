...
<ul class="operate">
	<li><a href="view.php<?=$view->parameter(array('parent'=>$hash['parent']['id']))?>&id=<?=$hash['data']['id']?>">前の画面に戻る</a></li>
...
<form class="content" method="post" name="history" action="">
	...
	<div class="submit">
		...
		<input type="button" value="キャンセル" onclick="location.href='view.php<?=$view->parameter(array('parent'=>$hash['parent']['id']))?>&id=<?=$hash['data']['id']?>'" />
	</div>
	<input type="hidden" name="id" value="<?=$hash['data']['id']?>" />
	<input type="hidden" name="customer_id" value="<?=$hash['data']['customer_id']?>" />
</form>
