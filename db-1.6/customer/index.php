...
$view->heading('顧客情報');
$pagination = new Pagination(array('folder'=>$_GET['folder'],'jufuku'=>$_GET['jufuku']));
$liquid = new Liquid;
?>
...
if ($view->permitted($hash['category'], 'add')) {
	echo '<li><a href="add.php?b_url=1&'.$view->parameter(array('folder'=>$_GET['folder'])).'">顧客追加</a></li>';
}
...
<li><a href="csv.php<?=$view->parameter(array('sort'=>$_GET['sort'], 'desc'=>$_GET['desc'], 'search'=>$_GET['search'], 'jufuku'=>$_GET['jufuku'], 'folder'=>$_GET['folder'], 'type'=>0))?>"<?=$attribute?>>一覧ダウンロード</a></li>

<li><a href="index.php<?=$view->parameter(array('folder'=>$_GET['folder'],'jufuku'=>1))?>">重複チェック</a></li>

</ul>
<?=$view->searchformime(array('folder'=>$_GET['folder']))?>

<table class="content" cellspacing="0">
<tr><td class="contentfolder">
