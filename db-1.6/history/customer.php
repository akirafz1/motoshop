...
?>
	...
	<tr><th>電話番号２</th><td><?=$hash['parent']['customer_fax']?>&nbsp;</td></tr>
	...
	<tr><th>担当者名</th><td><?=$hash['parent']['customer_workstaff']?>&nbsp;</td></tr>
</table>
...
<!--
	<li><a href="csv.php<?=$view->parameter(array('sort'=>$_GET['sort'], 'desc'=>$_GET['desc'], 'search'=>$_GET['search'], 'jufuku'=>$_GET['jufuku'], 'parent'=>$_GET['parent']))?>"<?=$attribute?>>CSV出力</a></li>
-->
...
if (is_array($hash['list']) && count($hash['list']) > 0) {
	foreach ($hash['list'] as $row) {
		...
		print $tr_tag;
?>
		<td><a href="view.php?id=<?=$row['id']?>&parent=<?=intval($_GET['parent'])?>">詳細</a></td>
		<td><?=$row['id']?></a>&nbsp;</td>
